...
class SRGAN:
    def __init__(self):
        # Input shape
        self.channels = 3
        self.lr_height = 8  # Low resolution height
        self.lr_width = 8  # Low resolution width
        ...
        self.dataset_name = 'E:/data/southgan/hrpic_pca'
    ...
    def build_generator(self):
        ...
        c2 = BatchNormalization(momentum=0.99)(c2)
        c2 = Add()([c2, c1])
    ...
    def test_images(self, batch_size=1):
        ...
        self.generator.load_weights('./saved_model/' + str(2500) + '.h5')
        fake_hr = self.generator.predict(imgs_lr)
...
if __name__ == '__main__':
    gan = SRGAN()
    # gan.train(epochs=3000, batch_size=25, sample_interval=2)
    j = 1
    for e in range(20):
        fake_hrs = gan.test_images()
        for i in range(fake_hrs.shape[0]):
            SaveImg = np.reshape(fake_hrs[i], (1024, 3))
            np.savetxt(os.getcwd() + '/pre/%d.csv' % j, SaveImg, delimiter=',')
            j = j+1
